...
#include "config.h"
 ...
 #else
  #include "pinout.h" // por defecto es placa MEGA
 #endif
#endif
...
void setup()
{
  ...
  #if SIMULATION && USE_ESP8266
    // Configura pines críticos ANTES de cualquier otra cosa
    pinMode(D4, OUTPUT);
    digitalWrite(D4, HIGH);
//
//    // Esperar un poco antes de tocar periféricos (boot estable)
    delay(500);  

    //Logger::logINFO("WiFi Config ...");
    // set the maximum tx power (Values range from 0 to 20.5 [dBm] inclusive)
    WiFi.setOutputPower(19.0);

    //Logger::logINFO("WiFi Off ...");
    // desconectar redes y apagar interfaces
    WiFi.disconnect(true);            // desconecta y borra credenciales
    //WiFi.softAPdisconnect(true);      // desconecta AP si estaba activo
    WiFi.mode(WIFI_OFF);     
  #endif
...
//******* logica modificada, para usar el driver sólo como gestor de comandos (simulacion)
  ...
  // Ejecutar un nuevo comando si Queue no esté vacía y interpolador haya terminado
  if ((!queue.isEmpty()) && interpolator.isFinished()) {
    executeCommand(queue.pop());
    if (PRINT_REPLY) {
      Serial.println(PRINT_REPLY_MSG);  // Imprime mensaje "OK"
    }
  }
